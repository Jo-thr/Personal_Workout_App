...
export const Card = ({ data }: CardProps) => {
  return (
    <Link href={data.slug}>
      <div className="group relative flex flex-col justify-end shadow-blur w-72 h-40 rounded-lg overflow-hidden hover:cursor-pointer">
        ...
        <div className="absolute w-full z-10 items-end flex flex-row justify-between text-white p-4">
          ...
          <ArrowRightCircleIcon className="w-6 h-6 group-hover:scale-125 transition-all group-hover:rotate-45 duration-500" />
        </div>
      </div>
    </Link>
  );
